...
export default function Produk({currentUser}) {
  ...
  const deleteAdmin = (id) => {
    services.deleteProduk(id)
      .then((response)=>{
        ...
        setPage(0);
        services.dataProduk(page,itemPerPage,cari)
          .then((response)=>{
  ...
  return (
    <div>
      ...
      <NavbarAndSidebar nama={currentUser.nama}>
        ...
        <h1 className="text-center m-5">Produk</h1>
        
        <div className="container">
          
          <div className="d-flex flex-column flex-md-row justify-content-between mb-3">
            <div>
              <Link href="/admin/produk/tambah_produk">
                <a type="button" className="btn btn-success">
                  <BsPlusLg/> Tambah Produk
                </a>
              </Link>
            </div>
            <div>
                ...
                { admins ?
                  admins.items.map((admin, index)=>
                    <tr key={index}>
                      ...
                      <td><img src={services.gambarProduk(admin.gambar_produk)} className="img-fluid" /></td>
                      <td>{admin.nama_produk}</td>
                      <td>{formatRupiah(admin.harga)}</td>
                      <td>{admin.stok}</td>
                      <td>
